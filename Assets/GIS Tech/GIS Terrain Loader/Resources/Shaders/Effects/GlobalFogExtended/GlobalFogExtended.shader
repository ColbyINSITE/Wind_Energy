Shader "Hidden/GlobalFogExtended"
{
    Properties
    {
        ...
        [MaterialToggle] _Scattering("Scattering", Float) = 0
    }
	...
	half4 ComputeFog (v2f i, bool distance, bool height, bool Scattering) : SV_Target
	{
        ...
        //float mPhase = hgPhase(cosTheta, -1000) * 1000;
        ...
        if(Scattering)
        {
            float rPhase = rayleighPhase(cosTheta * _Intensity);
            betaRTheta = float3(betaR * rPhase).xyz;
        }
        else
            betaRTheta = betaR * 0.8;

		//mPhase = hgPhase(cosTheta, -_Size);
...
SubShader
{
	...
	Pass
	{
		...
		#pragma target 3.0
		half4 frag (v2f i) : SV_Target { return ComputeFog (i, true, true, _Scattering); }
		ENDCG
	}
	// 1: distance
	Pass
	{
		...
		#pragma fragment frag
		#pragma target 3.0
		half4 frag (v2f i) : SV_Target { return ComputeFog (i, true, false, _Scattering); }
		ENDCG
	}
	// 2: height
	Pass
	{
		...
		#pragma fragment frag
		#pragma target 3.0
		half4 frag (v2f i) : SV_Target { return ComputeFog (i, false, true, _Scattering); }
		ENDCG
	}
}
